#!/usr/bin/env python3
"""
TCGPriceOracleV2 — Deploy + Register Script
Deploys the V2 oracle contract to Mantle Sepolia Testnet and registers 50 products.

Run from the Mac Mini:
  cd ~/undesirables-x402-server
  source venv/bin/activate
  python3 deploy_v2.py

Prerequisites:
  - web3.py installed (already in venv)
  - .env file with BURNER_PRIVATE_KEY
  - TCGPriceOracleV2_abi.json in same directory
  - TCGPriceOracleV2_bytecode.txt in same directory
  - market_memory.sqlite populated with card data
"""

import json
import os
import sys
import time
import sqlite3
from pathlib import Path
from web3 import Web3
from dotenv import load_dotenv

load_dotenv()

# ─── Configuration ─────────────────────────────────────────
RPC_URL = "https://rpc.sepolia.mantle.xyz"
CHAIN_ID = 5003
DB_PATH = os.path.expanduser("~/.cache/market_memory.sqlite")
ABI_PATH = os.path.join(os.path.dirname(__file__), "TCGPriceOracleV2_abi.json")
BYTECODE_PATH = os.path.join(os.path.dirname(__file__), "TCGPriceOracleV2_bytecode.txt")

# ─── Validation ────────────────────────────────────────────
def validate():
    """Check all prerequisites before doing anything."""
    errors = []
    
    key = os.getenv("BURNER_PRIVATE_KEY", "")
    if not key or key == "your_private_key_here":
        errors.append("BURNER_PRIVATE_KEY not set in .env")
    
    if not os.path.exists(ABI_PATH):
        errors.append(f"ABI file not found: {ABI_PATH}")
    
    if not os.path.exists(BYTECODE_PATH):
        errors.append(f"Bytecode file not found: {BYTECODE_PATH}")
    
    if not os.path.exists(DB_PATH):
        errors.append(f"Database not found: {DB_PATH}")
    
    if errors:
        print("\n  PREFLIGHT FAILED:")
        for e in errors:
            print(f"    ✗ {e}")
        print()
        sys.exit(1)


def get_top_50_products(db_path):
    """Query SQLite for the top 50 products by market price."""
    conn = sqlite3.connect(db_path)
    cursor = conn.cursor()
    
    # Get the latest date in price_history
    latest_date = cursor.execute(
        "SELECT MAX(date) FROM price_history"
    ).fetchone()[0]
    
    # Top 50 by market price, joining cards for names
    rows = cursor.execute("""
        SELECT 
            p.product_id,
            c.category_id,
            c.name,
            CAST(p.market_price * 100 AS INTEGER),
            CAST(p.low_price * 100 AS INTEGER)
        FROM price_history p
        JOIN cards c ON p.product_id = c.product_id
        WHERE p.date = ?
          AND p.market_price > 0
        ORDER BY p.market_price DESC
        LIMIT 50
    """, (latest_date,)).fetchall()
    
    conn.close()
    print(f"  Loaded {len(rows)} products from {latest_date}")
    return rows


def deploy():
    """Deploy TCGPriceOracleV2 and register initial products."""
    validate()
    
    # Connect
    w3 = Web3(Web3.HTTPProvider(RPC_URL, request_kwargs={"timeout": 60}))
    if not w3.is_connected():
        print("  ERROR: Cannot connect to LiteForge RPC")
        sys.exit(1)
    
    # Load wallet
    private_key = os.getenv("BURNER_PRIVATE_KEY").strip()
    if not private_key.startswith("0x"):
        private_key = "0x" + private_key
    account = w3.eth.account.from_key(private_key)
    wallet = account.address
    
    balance = w3.eth.get_balance(wallet)
    balance_eth = w3.from_wei(balance, "ether")
    
    print("\n" + "=" * 60)
    print("  TCGPriceOracleV2 — Deployment")
    print("=" * 60)
    print(f"  Network:  Mantle Sepolia Testnet (Chain ID {CHAIN_ID})")
    print(f"  Wallet:   {wallet}")
    print(f"  Balance:  {balance_eth} MNT")
    
    if balance == 0:
        print("\n  ERROR: No MNT. Get tokens from https://faucet.testnet.mantle.xyz")
        sys.exit(1)
    
    # Load ABI and bytecode
    with open(ABI_PATH) as f:
        abi = json.load(f)
    with open(BYTECODE_PATH) as f:
        bytecode = f.read().strip()
    
    # ─── Step 1: Deploy ────────────────────────────────────
    print("\n  [1/3] Deploying contract...")
    
    contract = w3.eth.contract(abi=abi, bytecode=bytecode)
    
    nonce = w3.eth.get_transaction_count(wallet)
    tx = contract.constructor().build_transaction({
        "chainId": CHAIN_ID,
        "from": wallet,
        "nonce": nonce,
        "gas": 5000000,
        "gasPrice": w3.eth.gas_price,
    })
    
    signed = w3.eth.account.sign_transaction(tx, private_key)
    tx_hash = w3.eth.send_raw_transaction(signed.raw_transaction)
    print(f"         TX: {tx_hash.hex()}")
    
    receipt = w3.eth.wait_for_transaction_receipt(tx_hash, timeout=120)
    
    if receipt.status != 1:
        print("  ERROR: Deployment failed!")
        sys.exit(1)
    
    contract_address = receipt.contractAddress
    print(f"  ✅ Deployed: {contract_address}")
    
    # ─── Step 2: Register Products ─────────────────────────
    print("\n  [2/3] Registering initial products...")
    
    oracle = w3.eth.contract(address=contract_address, abi=abi)
    products = get_top_50_products(DB_PATH)
    
    # Batch in groups of 25
    BATCH_SIZE = 25
    nonce = w3.eth.get_transaction_count(wallet)
    
    for i in range(0, len(products), BATCH_SIZE):
        batch = products[i:i + BATCH_SIZE]
        batch_num = (i // BATCH_SIZE) + 1
        total_batches = (len(products) + BATCH_SIZE - 1) // BATCH_SIZE
        
        ids = [p[0] for p in batch]
        cats = [p[1] for p in batch]
        names = [p[2] for p in batch]
        prices = [p[3] for p in batch]
        lows = [p[4] for p in batch]
        
        print(f"         Batch {batch_num}/{total_batches} ({len(batch)} products)...")
        
        tx = oracle.functions.batchRegister(
            ids, cats, names, prices, lows
        ).build_transaction({
            "chainId": CHAIN_ID,
            "from": wallet,
            "nonce": nonce,
            "gas": 8000000,
            "gasPrice": w3.eth.gas_price,
        })
        
        signed = w3.eth.account.sign_transaction(tx, private_key)
        tx_hash = w3.eth.send_raw_transaction(signed.raw_transaction)
        receipt = w3.eth.wait_for_transaction_receipt(tx_hash, timeout=120)
        
        if receipt.status != 1:
            print(f"  ERROR: Batch {batch_num} failed! TX: {tx_hash.hex()}")
            sys.exit(1)
        
        nonce += 1
        print(f"         ✅ Batch {batch_num} confirmed (gas: {receipt.gasUsed})")
    
    # ─── Step 3: Verify ───────────────────────────────────
    print("\n  [3/3] Verifying on-chain state...")
    
    product_count = oracle.functions.productCount().call()
    total_updates = oracle.functions.totalUpdates().call()
    owner = oracle.functions.owner().call()
    
    print(f"         Products:      {product_count}")
    print(f"         Total updates: {total_updates}")
    print(f"         Owner:         {owner}")
    
    # Spot check
    try:
        price, ts, fresh = oracle.functions.getLatestPrice(products[0][0]).call()
        print(f"         Top product:   ${price / 100:,.2f} (fresh: {fresh})")
    except Exception as e:
        print(f"         Spot check:    {e}")
    
    # ─── Save results ─────────────────────────────────────
    result = {
        "contract_address": contract_address,
        "chain_id": CHAIN_ID,
        "deployer": wallet,
        "product_count": product_count,
        "total_updates": total_updates,
        "deployed_at": int(time.time()),
        "explorer": f"https://explorer.sepolia.mantle.xyz/address/{contract_address}",
    }
    
    result_path = os.path.join(os.path.dirname(__file__), "mantle_v2_deployment.json")
    with open(result_path, "w") as f:
        json.dump(result, f, indent=2)
    
    print("\n" + "=" * 60)
    print("  DEPLOYMENT COMPLETE")
    print("=" * 60)
    print(f"  Contract:  {contract_address}")
    print(f"  Explorer:  {result['explorer']}")
    print(f"  Products:  {product_count}")
    print(f"  Saved to:  {result_path}")
    print("=" * 60)
    print("\n  NEXT: Update mantle_updater_v2.py with the new contract address.")
    print("        Then update the crontab to use mantle_updater_v2.py\n")


if __name__ == "__main__":
    deploy()
