...
def deploy():
    ...
    print("=" * 60)
    print(f"  Network:  LitVM LiteForge (Chain ID {CHAIN_ID})")
    print(f"  Wallet:   {wallet}")
    print(f"  Balance:  {balance_eth} zkLTC")
    
    if balance == 0:
        print("\n  ERROR: No zkLTC. Get tokens from https://liteforge.hub.caldera.xyz")
        sys.exit(1)
    ...
    result_path = os.path.join(os.path.dirname(__file__), "v2_deployment.json")
    ...
    print("=" * 60)
    print("\n  NEXT: Update litvm_updater_v2.py with the new contract address.")
    print("        Then update the crontab to use litvm_updater_v2.py\n")
